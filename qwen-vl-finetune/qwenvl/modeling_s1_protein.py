...
class S1Protein(nn.Module):
    ...
    def _resolve_esm_model_source(model_name_or_path: str):
        if isinstance(model_name_or_path, str) and os.path.isdir(model_name_or_path):
            ...
            if (
                ...
                and S1Protein._esm2_model_source_looks_loadable(esm_dir)
            ):
                return esm_dir
        return model_name_or_path

    @staticmethod
    def _esm2_model_source_looks_loadable(model_dir: str) -> bool:
        ...
        def collect_module_name(value):
            if isinstance(value, str):
                local_spec = value.rsplit("--", 1)[-1]
                module_name = local_spec.split(".", 1)[0]
                if module_name:
                    required_modules.add(module_name)
            elif isinstance(value, (list, tuple)):
                for item in value:
                    collect_module_name(item)

        for target in auto_map.values():
            collect_module_name(target)

        if not required_modules:
            return True
        return all(
            os.path.exists(os.path.join(model_dir, f"{module_name}.py"))
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if (
            ...
            and cls._esm2_model_source_looks_loadable(bundled_esm_dir)
        ):
    ...
    def _load_esm2_from_pretrained(
        ...
        rejected_unexpected_key = 0
        for key, value in state_dict.items():
            if key not in current_state_dict:
                rejected_unexpected_key += 1
                continue
        ...
        if require_bundled and (
            ...
            raise RuntimeError(
                ...
                f"rejected unexpected_key={rejected_unexpected_key}, "
    ...
    def _load_head_from_pretrained(self, model_name_or_path: str):
        ...
        if not module_state_dict:
            ...
            print(
                f"[S1Protein] custom module LOAD REPORT from: {model_name_or_path}\n"
                "  no S1Protein custom module tensors found."
            )
            return report
        ...
        report = {
            "source": model_name_or_path,
            "loaded_tensors": 0,
            "checkpoint_tensors": len(module_state_dict),
            "expected_tensors": 0,
            "modules": {},
            "missing_keys": [],
            "shape_mismatches": [],
        ...
        for module_name in (
            ...
            report["expected_tensors"] += len(current_state_dict)
            ...
            module_missing_keys = [
                ...
                if f"{prefix}{key}" not in module_state_dict
            ]
            module_shape_mismatches = [
                {
                    "key": key,
                    "checkpoint_shape": tuple(value.shape),
                    "model_shape": tuple(current_state_dict[key.removeprefix(prefix)].shape),
                }
                for key, value in module_state_dict.items()
                if key.startswith(prefix)
                and key.removeprefix(prefix) in current_state_dict
            ...
            report["missing_keys"].extend(module_missing_keys)
            report["shape_mismatches"].extend(module_shape_mismatches)
            report["modules"][module_name] = {
                "loaded": len(compatible_state_dict),
                "expected": len(current_state_dict),
                "checkpoint": len(module_checkpoint_keys),
                "missing": len(module_missing_keys),
                "shape_mismatches": len(module_shape_mismatches),
            }
            if compatible_state_dict:
                module.load_state_dict(compatible_state_dict, strict=False)
                loaded_keys = {f"{prefix}{key}" for key in compatible_state_dict}
                consumed_keys.update(loaded_keys)
                report["loaded_tensors"] += len(compatible_state_dict)

        report["unexpected_keys"] = sorted(set(module_state_dict) - consumed_keys)
        lines = [f"[S1Protein] custom module LOAD REPORT from: {model_name_or_path}"]
        for module_name, module_report in report["modules"].items():
            lines.append(
                f"  {module_name}: loaded {module_report['loaded']}/{module_report['expected']} "
                f"(checkpoint={module_report['checkpoint']}, "
                f"missing={module_report['missing']}, "
                f"shape_mismatch={module_report['shape_mismatches']})"
            )
        lines.append(
            f"  total: loaded {report['loaded_tensors']}/{report['expected_tensors']} "
        ...
        if report["shape_mismatches"]:
            preview = ", ".join(item["key"] for item in report["shape_mismatches"][:5])
            lines.append(f"  shape mismatch preview: {preview}")
        if report["missing_keys"]:
            preview = ", ".join(report["missing_keys"][:5])
            lines.append(f"  missing key preview: {preview}")
        print("\n".join(lines))
        return report
    ...
    def save_pretrained(self, save_directory: str, **kwargs):
        ...
        for key, value in self.protein_head.state_dict().items():
            combined_state_dict.setdefault(f"protein_head.{key}", value)
        if self.use_esm2:
            for module_name in (
                "esm_query_projection",
                "qwen_key_projection",
                ...
                for key, value in module.state_dict().items():
                    combined_state_dict.setdefault(f"{module_name}.{key}", value)
